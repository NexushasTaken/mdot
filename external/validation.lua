...
--- Generates one_of meta validator.
--
-- Allow data validation using multiple validators and applying
-- OR condition between their results.
--
-- Validation succeeds if at least one validator succeeds.
--
-- @param ... function
--   Variable number of validator functions.
--
-- @return
--   One_of validator function.
--   This validator returns true on success or false and a string
--   describing the combined validation errors.
---
function M.one_of(...)
   local validators = {...}
   ...
   return function(value, key, data)
      ...
      for _, validator in ipairs(validators) do
         local valid, err = validator(value, key, data)
         if valid then
            return true
         end
         if err then
            if type(err) == "table" then
               insert(errors, M.print_err(err))
            else
               insert(errors, err)
            end
         end
...
--- Generates table_key_value validator.
--
-- Validate table data by applying validators on both keys and values.
--
-- @param key_validator function
--   Validator function to check each key.
-- @param value_validator function
--   Validator function to check each value.
--
-- @return
--   Table validator function.
--   This validator returns true on success or false and a nested table
--   holding all key/value errors.
---
function M.table_key_value(key_validator, value_validator)
   return function(value, key, data)
      local errs = {}
      ...
      for k, v in pairs(value) do
         -- Validate key
         local ok, err = key_validator(k, k, value)
         if not ok then
            errs[k] = { key = err }
         end

         -- Validate value
         ok, err = value_validator(v, k, value)
         if not ok then
            if errs[k] then
               errs[k].value = err
            else
